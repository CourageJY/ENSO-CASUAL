...
sst = np.load(f"{params.final_data_dir}/reanalysis/sst-final.npz")['sst']
...
model.load_weights(f'{params.encoder_save_dir}/reanalysis/sst-model')

sst_encoder=np.load(f'{params.encoder_save_dir}/reanalysis/sst-encoder.npz')['sst']
#print(sst_encoder[0].reshape(64).shape)
t = tf.convert_to_tensor(sst_encoder, tf.float32)

# print(tf.reshape(t[0],[1,64]).shape)
# print(t.shape)
res=model.decoder((tf.reshape(t[6],[1,64])))
